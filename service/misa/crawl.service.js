...
function MisaCrawlService() {
  const SELF = {
    ...
    mapOmisellToCrmSaleOrder(src, pickups = []) {
        ...
        const pickupInfo = firstParcel?.pickup_id ? pickups.find(p => p.pickup_id === firstParcel.pickup_id) || {} : {};
        ...
        return {
            ...
            sale_order_type: src.order_type || null,
    ...
    addCrmObjects: async (select, items, token, clientId, crmUrl = SELF.AMIS_CRM_URL) => {
        try {
            console.log(`[MisaCrawlService] - [addCrmObjects] - select=${select} - token=${token}`);
            ...
            const data = await response.json();
            console.log(`[MisaCrawlService] - [addCrmObjects] - data=${JSON.stringify(data)}`);
            return Promise.resolve(data);
        } catch (error) {
            console.log(`[MisaCrawlService] - [addCrmObjects] - fail: `, error.stack);
            return Promise.reject(error);
        }
    }
  }
  return {
    createOrder: async () => {
        try {
            await SELF.addCrmObjects('SaleOrders', items, token, clientId, crmUrl)
        } catch (error) {
           console.log(`[MisaCrawlService] - [createOrder] - fail: `, error.stack);
           return Promise.reject(error);
        }
    },
  }
...
module.exports = new MisaCrawlService()
